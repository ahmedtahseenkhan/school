...
const sections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      { to: '/admin/permissions/roles', label: 'Roles & Permissions', icon: ShieldCheckIcon, anyOf: ['role:read', '*:manage'] },
  ...
  },
  {
    label: 'Super Admin',
    items: [
      { to: '/admin/super-admin/dashboard', label: 'SaaS Dashboard', icon: ShieldCheckIcon },
      { to: '/admin/super-admin/branches', label: 'Branch Management', icon: BriefcaseIcon },
      { to: '/admin/super-admin/modules', label: 'Module Licensing', icon: ShieldCheckIcon },
      { to: '/admin/super-admin/server', label: 'Server Monitor', icon: Cog6ToothIcon }
    ]
  }
];
...
export function Sidebar({ open = false, onClose }: { open?: boolean; onClose?: () => void }) {
  ...
  const visibleSections = React.useMemo(() => {
    ...
    return sections
      .filter(section => {
        if (section.label === 'Super Admin') return (user as any)?.role === 'super_admin';
        return true;
      })
      .map((section) => ({
        ...section,
