...
CREATE TABLE IF NOT EXISTS job_applications (
    ...
    screening_by UUID REFERENCES users(id),
    screening_date DATE,
...
CREATE TABLE IF NOT EXISTS interviews (
    ...
    -- Interview Details
    interview_type VARCHAR(50) DEFAULT 'TECHNICAL', -- TECHNICAL, HR, MANAGER, FINAL
    interview_round INTEGER DEFAULT 1,
    scheduled_at TIMESTAMP NOT NULL,
    duration_minutes INTEGER DEFAULT 60,
    location VARCHAR(255), -- Meeting room or online link
...
CREATE TABLE IF NOT EXISTS interview_panels (
    ...
    panel_member_id UUID NOT NULL REFERENCES employees(id) ON DELETE CASCADE,
...
-- Add Comments
COMMENT ON TABLE applicants IS 'External applicants for job postings';
COMMENT ON TABLE job_applications IS 'Links applicants to job postings with application tracking';
COMMENT ON TABLE interviews IS 'Interview schedules and details';
